...
if(isset($_POST['form1'])) {
	...
	if($valid == 1) {
		...
		if($path == '' && $path1 == '') {
			$statement = $pdo->prepare("UPDATE tbl_service SET name=?, slug=?, description=?, short_description=? WHERE id=?");
		...
		if($path != '' && $path1 == '') {
        	...
        	$statement = $pdo->prepare("UPDATE tbl_service SET name=?, slug=?, description=?, short_description=?, photo=? WHERE id=?");
		...
		if($path == '' && $path1 != '') {
        	...
        	$statement = $pdo->prepare("UPDATE tbl_service SET name=?, slug=?, description=?, short_description=?, banner=? WHERE id=?");
		...
		if($path != '' && $path1 != '') {
        	...
        	$statement = $pdo->prepare("UPDATE tbl_service SET name=?, slug=?, description=?, short_description=?, photo=?, banner=? WHERE id=?");
...
<section class="content-header">
	...
	<div class="content-header-right">
		<a href="service.php" class="btn btn-primary btn-sm">Visualizar Todos</a>
	</div>
</section>
				...
				<div class="box box-info">
					<div class="box-body">
						...
						<div class="form-group">
							...
							<div class="col-sm-9">
								<textarea id="1" class="form-control" name="description"><?php echo $description; ?></textarea>
							</div>
						</div>
						<div class="form-group">
							<label for="" class="col-sm-2 control-label">Descrição Curta <span>*</span></label>
							<div class="col-sm-9">
								<textarea id="2" class="form-control" name="short_description" style="height:140px;"><?php echo $short_description; ?></textarea>
							</div>
						</div>
						<div class="form-group">
							<label for="" class="col-sm-2 control-label">Foto Existente</label>
							<div class="col-sm-9" style="padding-top:5px">
								<img src="<?php echo BASE_URL; ?>assets/uploads/<?php echo $photo; ?>" alt="Foto do Serviço" style="width:400px;">
							</div>
						</div>
						<div class="form-group">
							<label for="" class="col-sm-2 control-label">Foto </label>
							<div class="col-sm-6" style="padding-top:5px">
								<input type="file" name="photo">(Somente jpg, jpeg, gif e png serão permitidas)
							</div>
						</div>
						<div class="form-group">
							<label for="" class="col-sm-2 control-label">Banner Existente</label>
							<div class="col-sm-9" style="padding-top:5px">
								<img src="<?php echo BASE_URL; ?>assets/uploads/<?php echo $banner; ?>" alt="Foto Banner do Serviço" style="width:400px;">
							</div>
						</div>
						<div class="form-group">
							<label for="" class="col-sm-2 control-label">Banner </label>
							<div class="col-sm-6" style="padding-top:5px">
								<input type="file" name="banner">(Somente jpg, jpeg, gif e png serão Permitdas)
							</div>
						</div>		
						<div class="form-group">
