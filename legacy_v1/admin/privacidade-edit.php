...
if(isset($_POST['form1'])) {
    ...
    if($_POST <> NULL){
        $statement = $pdo->prepare("UPDATE tbl_privacidade SET titulo = ?, detalhes = ?,status = ? WHERE id = ?");
		$statement->execute($dados);
		

		$success_message = 'Privacidade Atualizada com Sucesso!';
    }
...
<?php
   ...
   if(isset($_POST["UpdateBanner"])){
    ...
    if($path == '') {
        ...
        if( $ext!='jpg' && $ext!='png' && $ext!='jpeg' && $ext!='gif' ) {
            ...
            $_SESSION['erro'] = 'Você deve ter que fazer upload de jpg, jpeg, gif ou png arquivo para banner<br>';
             header('location: privacidade.php');
        }
    ...
    if($path != '') {
            ...
            $statement = $pdo->prepare("SELECT * FROM tbl_privacidade WHERE id=?");
    		...
    		foreach($result as $row) {
    		    ...
    		    unlink('../assets/uploads/privacidade/' . $banner);

			$final_name = 'privacidade-' . $row['id'] . '.' . $ext;
        	move_uploaded_file( $path_tmp, '../assets/uploads/privacidade/'.$final_name );

        	$statement = $pdo->prepare("UPDATE tbl_privacidade SET banner=? WHERE id=?");
				...
				<div class="box box-info">
					<div class="box-body">
						<div class="form-group">
							...
							<div class="col-sm-6">
								<input type="text" autocomplete="off" class="form-control" name="titulo" value="<?php echo $name; ?>">
							</div>
						</div>
						<div class="form-group">
